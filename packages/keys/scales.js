_Scales = new Mongo.Collection('scales');
_Keys = new Mongo.Collection('keys');
...
if (Meteor.isServer) {
  ...
  Meteor.startup(function() {
    if (_Scales.find().count() > 0 || _Keys.find().count() > 0) {
      return;
    }
    ...
    console.log('Initialized ' + _Scales.find().count() + 'Scales');
